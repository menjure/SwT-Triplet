...
def window_partition(x, window_size: int):
    ...
    # permute: [B, H//Wh, Wh, W//Ww, Ww, C] -> [B, H//Wh, W//Wh, Ww, Ww, C]
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        # [batch_size*num_windows, Wh*Ww, total_embed_dim]
        B_, N, C = x.shape
        ...
        if mask is not None:
            ...
            attn = attn.view(-1, self.num_heads, N, N)  # [nW*batch_size, num_heads, Wh*Ww, Wh*Ww]
...
class BasicLayer(nn.Module):
    ...
    def create_mask(self, x, H, W):
        ...
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)  # [nW, 1, Wh*Ww] - [nW, Wh*Ww, 1]
        # [nW, Wh*Ww, Wh*Ww]
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
        return attn_mask

    def forward(self, x, H, W):
        attn_mask = self.create_mask(x, H, W)  # [nW, Wh*Ww, Wh*Ww]
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, embedding_size=128, **kwargs):
        super().__init__()
        ...
        self.fc_embedding = nn.Linear(self.num_features, embedding_size)
        self.norm_ = nn.LayerNorm(embedding_size)
        self.fc_softmax = nn.Linear(embedding_size, num_classes)
